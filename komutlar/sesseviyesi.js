...
if(!message.member.voice.channel) return message.channel.send({embed: {color: FynxHata, description: `<:x_x:751923344407789579>| Ses seviyesini ayarlabilmek için bir ses kanalında olmanız gerekmektedir!` }})  
...
if (!sesseviyesi) return message.channel.send({embed: {color: FynxHata, description: `<:x_x:751923344407789579>| Ses seviyesini ayarlayabilmek için sadece bir doğal sayı giriniz.` }})
...
message.channel.send({embed: {color: FynxDogru, description: `<a:tik:751910144996016200>| Ses seviyesi, başarılı bir şekilde \`${args.join(" ")}\` olarak ayarlandı! ` }})
}
